...
nflg_tree <- read.beast('./2024Apr21/USUV_nflg_2024May7_subsampled_mcc') %>% left_join(., metadata, by = join_by(label == tipnames))%>%
  left_join(., 
...
eu3_tree <- read.beast('~/Downloads/USUV_EU3_nflg_2024May7_subsampled_mcc.tree') %>% left_join(., metadata, by = join_by(label == tipnames))

ggtree(eu3_tree, 
       mrsd="2021-01-01", right = TRUE) + 
  geom_tippoint(aes( color=host_order))  +
  #geom_tiplab(aes(label = collection_countrycode)) +
  theme_tree2() + 
  theme(axis.text =  element_text(size=18))
