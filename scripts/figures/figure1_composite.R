...
# B - proportions of lineages across countries
lineage_props_country <- lineage_tbl %>%
  left_join(metadata %>% select(label = tipnames, starts_with('nuts0'), is_europe)) %>%
  filter(is_europe ==1 ) %>%
  ggplot() + 
  geom_bar(aes(x = nuts0_id, fill = `GRI Lineage Level 1`), position = 'fill', alpha = 0.8) +
  scale_colour_manual('Lineage', aesthetics = 'colour',values = lineage_colours)+
  scale_fill_manual('Lineage', aesthetics = 'fill',
                    values = lineage_colours)  +
  scale_x_discrete('Country', expand = c(0.05,0.05)) +
...
# C
lineage_seqs_overtime <-lineage_tbl %>%
  left_join(metadata %>% select(label = tipnames, date_y, is_europe)) %>%
  filter(is_europe ==1 ) %>%
  ggplot() + 
  geom_histogram(aes(fill = `GRI Lineage Level 1`, x = date_y), binwidth = 2, alpha = 0.8) +
  scale_colour_manual('Lineage', aesthetics = 'colour',values = lineage_colours)+
  scale_fill_manual('Lineage', aesthetics = 'fill',
                    values = lineage_colours)  +
  scale_x_continuous('Date', limits = c(1999, 2025), expand = c(0,0), breaks = seq(2000, 2025, by = 5),
...
lineage_legend <- get_legend(lineage_props_country)
plt_bttm <- plot_grid(lineage_legend, lineage_props_country +
            theme(legend.position = 'none') , 
          lineage_seqs_overtime +
            theme(legend.position = 'none') , nrow = 1, align = 'v', axis = 'tb', labels = c('','B', "C"), label_size = 9, rel_widths = c(0.5, 1,1))

plot_grid(main, '',plt_bttm, nrow = 3, labels = c('A','', ''), rel_heights = c(0.8, 0.01, 0.2), label_size = 9)
ggsave('./2025Jun24/plots/figure1.pdf', height = 27, width = 22, units = 'cm', dpi =360)
